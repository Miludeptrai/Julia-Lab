...
function entropy(counts, n_samples)
    ...
    =#
    #TODO
	_entropy_ = filter(x->x>0,counts)./n_samples
	_entropy_ = -sum(log2.(_entropy_) .*_entropy_)
	return _entropy_
end
...
function plot_pdf(self::NBGaussian)
    #plot Histogram
    #TODO
	x = [i for i in keys(self.hist)]
	y = [i for i in values(self.hist)]
	p = bar(x,y)
	#ylabel!("")
	xlabel!("Class")
	title!("Xác xuất thuộc vào các class")
end
...
function maxHypo(self::NBGaussian)
    ...
    #TODO
	max = maximum(values(self.hist))
	return [i for i in keys(self.hist) if self.hist[i]==max][1]
end
...
function likelihood(self::NBGaussian,data, hypo)
	...
	#Hàm Gauss được thiết kế để truyền cả vector vào vẫn đúng 
		gauss =  Gauss(std, mean, data)
		for i in gauss 
			res = res * i
		end
        return res
...
function update(self::NBGaussian, data) 
    ...
    for hypo in keys(self.hist)
        #self.hist[hypo]=?
		self.hist[hypo]=likelihood(self,data,hypo) * self.hist[hypo]
	end
	#Normalization
        
    #TODO: s=P(data)
    #s=?
    s = sum(values(self.hist))
...
function fit_1(self::NBGaussian, X,y)
        ...
        for hypo in range(0, length = n_species)
            ...
            #TODO rows=
            rows = [X[i] for i in 1:n if y[i]==hypo]
...
# ╔═╡ a0acb78f-8d94-4857-b8d5-9f2b21fa9560
begin 	
	_unique_ = unique(vcat(y_test , pred1))
	println("\t\t" , "precision","\t","recall","\t","f1-score")
	for i in sort(_unique_)
		y_t = [ x==i ? 1 : 0 for x in y_test]
		y_p = [ x==i ? 1 : 0 for x in pred1]
		TP, FP, TN, FN = 0, 0, 0, 0
		
		for x in 1:length(y_t)
		    if y_t[x] == 1 && y_p[x] == 1
		        TP += 1
			elseif y_t[x] == 0 && y_p[x] == 1
		        FP += 1
			elseif y_t[x] == 0 && y_p[x] == 0
		        TN += 1
			elseif y_t[x] == 1 && y_p[x] == 0
		        FN += 1
			end
		end
		precision = TP / (TP+FP)
		...
		println(i,"\t\t",round(precision, digits =2)," \t\t",round(recall, digits =2)," \t",round(f1, digits =2))
	end 
end

# ╔═╡ 48d5d8bd-97f7-4489-8630-4fd26ad3447f
md"""
Các giá trị precision, recall và f1-score cho thấy mô hình rất tốt trên tổng thể, đạt điểm số cao trên tất cả các chỉ số đo lường.

Lớp 0, mô hình đạt được độ chính xác và độ nhớ lại hoàn hảo, nó đã xác định đúng tất cả các trường hợp của lớp này và không có bất kỳ dự đoán sai tích cực hoặc tiêu cực nào.

Lớp 1, recall là 0.89 , điểm số f1 là 0.94 cho thấy rằng mô hình đã đạt được một sự cân bằng tốt giữa độ chính xác và độ nhớ lại cho lớp này.

Lớp 2, precision là 0.87 cho thấy rằng có một số trường hợp đã được xác định sai là thuộc về lớp này. Điểm số f1 là 0.93 cho thấy một hiệu suất tổng thể tốt cho lớp này.
"""
...
manifest_format = "2.0"
project_hash = "d4283e40bded111d2303ba6cce7ab740e75f7203"
