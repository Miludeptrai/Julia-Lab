...
function compute_average_intensity(x, y)
	...
	count_ = zeros(10)
	for i in 1:size(x)[1]
		mean_[y[i]+1] = mean_[y[i]+1] + sum(x[i,:])/size(x)[2]
		count_[y[i]+1] = count_[y[i]+1] + 1
	end
	mean_ = mean_ ./ count_
    return mean_
end
...
- **TODO: Study about Z-score normalization**
	 + Z-score = (x - trung bình) / độ lệch chuẩn
	 + Z-score bảo tồn hình dạng của phân phối ban đầu
	 + Diễn giải của dữ liệu được chuyển đổi Z-score là rõ ràng, vì nó được biểu thị dưới dạng đơn vị độ lệch chuẩn từ giá trị trung bình
- **TODO: Why should we normalize data?**
	+ Tránh thiên vị: đảm bảo rằng tất cả các thuộc tính có ảnh hưởng bằng nhau đến kết quả.
	+ Đơn giản hóa quá trình tối ưu hóa: quá trình tối ưu hóa có thể nhanh hơn và chính xác hơn vì mô hình có thể hội tụ nhanh chóng.
"""
...
function compute_gradient(error, train_x)
    ...
    """
    return Base.transpose(train_x) * error ./size(train_x)[1]
end
...
begin
	...
	normalized_test_x, mean__, std__ = normalize(test_x_new)
...
+ Không tránh được các kiểu viết số "quái dị" nên mô hình chắc chắn khồng thể nào đạt mức tốt (với tiêu chuẩn  f1_score>0.7) chỉ với hai thuộc tính cho mỗi ảnh là  intensity và symmetry. Ít nhất ta được : 
	+ precision: ~0.8 -> cứ 5 trong số các ảnh được đoán là chữ số 1 thì có 4 ảnh là đúng thật vậy
	+ f1_score: ~0.5 -> độ tin cậy của mô hình mức tạm được 
	+ Lý do chính giải thích cho recall thấp có lẽ là một có hai cách viết chăng? 1 và "|" 
+ Đề xuất cái thiện symmetry, nếu chỉ dùng reverse(<img>, dims=2) sau đó tính toán sự khác biệt % giữa các <img> và reverse(<img>, dims=2). 
+ Đề xuất cái thiện intensity, bật sáng tối đa các giá trị lớn hơn 0 [1 if x>0 else 0]
"""
...
manifest_format = "2.0"
project_hash = "937cbca7e7355176bfbd6a26fbf3df429e447f1c"
