...
begin
	...
	#TODO
	#Hàm plot trong bài chưa được định nghĩa 
	using Plots
end
...
$h = XW \Rightarrow \frac{\partial h}{\partial W} = \frac{\partial XW}{\partial W} = X$



$\hat{y} = \sigma(h) \Rightarrow \frac{\partial \hat{y}}{\partial h} = \frac{\partial \sigma(h)}{\partial h}= \frac{\partial (\frac{1}{1 + e^{-h}})}{\partial h} = \frac{e^{-h}}{({1 + e^{-h})^2}} = \sigma(h)({1 - \sigma(h)})$


$Loss = \frac{1}{2}(\hat{y} - y)^2 \Rightarrow \frac{\partial Loss}{\partial \hat{y}} = \frac{\partial (\frac{1}{2}(\hat{y} - y)^2)}{\partial \hat{y}} = \hat{y} - y$
...
begin
	function compute_gradient(error, y_hat, trainX)
	    ...
	    """
	    # return?
		return Base.transpose(trainX)*error
	end
end
...
begin
	...
	#Note thay đổi W thành W1 
	W1 = rand(Normal(), (size(X_train)[2], 1))
	num_epochs=100
	learning_rate=0.001
end
...
begin
	#training model
	W, losses = train(W1, X_train, y_train, learning_rate, num_epochs)
...
- Accuracy: tỉ lệ giữa tổng số điểm được dự đoán positive thực sự là positive và số điểm được dự đoán negative thực sự là negative tất cả chia cho số điểm trong tập dự đoán.
- Recall (aka sensitivity hay specificity): tỷ lệ số điểm positive được dự đoán positive bởi mô hình. Recall cao cho thấy mô hình xác định tốt các điểm positive.
- Precision (aka positive predictive value): là tỉ lệ số điểm được dự đoán là positive thực sự là positive trên tổng số những điểm được dự đoán là positive. Precision là một số liệu hữu ích và cho thấy rằng trong số những dự đoán được dự đoán là tích cực, dự đoán đó chính xác đến mức nào
- F1: (F1-score) là chỉ số đánh giá học máy để đo lường độ chính xác của mô hình. Là harmonic mean của precision và recall. Được dùng khi Precision hay Recall được ưu tiên như nhau. Với độ ưu tiên khác nhau, dùng Fβ-score.
"""

# ╔═╡ 7afa4949-ab50-4276-8e37-109c98f8bd6f
md"""
**TODO: Try out different learning rates. Give me your observations**
+ learning-rate = 0.5: Loss giảm nhanh sau đó dường như giao động lên xuống trong một khoảng rõ ràng.
+ learning-rate = 0.001: Loss giảm nhanh, sau đó đồ thị mịn thoạt nhìn như đường thẳng. 
+ learning-rate = 0.0001: Loss giảm dần, giảm nhanh ở các epcho đầu và sau đó chậm dần, đồ thị tựa hình cầu trượt. Mô hình dự đoán gần như chính xác. 
+ learning-rate = 0.000001: Loss giảm dần, nhưng tốc độ giảm của nó rất chậm, đồ thị là một cung cong.

#Giải thích : về bản chất Gradient Descent chính là tìm hướng xuống dốc hiệu quả nhất, learning-rate chính là chiều dài mỗi bước chân xuống dốc, bước chân càng nhỏ thời gian xuống dốc càng lâu, ngược lại bước chân càng lớn điểm thấp có thể vừa được bỏ qua sau đó lại phải đi ngược lại hướng vừa đi dẫn đến "Hàm mất mát" tăng giảm thất thường.
...
manifest_format = "2.0"
project_hash = "9a41a2d03fa8c287f61313ab2dd4b63d9be92b1c"
